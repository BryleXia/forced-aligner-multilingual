...
> 把字幕文件的时间戳准确对齐到音频上。提供两套互补方案，覆盖不同录音质量场景。
> 支持 **西班牙语、法语、俄语**，切换语言只需修改一行配置。
> 方案 A 在 5 段纪录片音频（390 条字幕，约 56 分钟）上经过人工逐句审听，**0 条错误**。

**作者：** BryleXia · 北京第二外国语学院欧洲学院  
**核心脚本：** `align_srt_routeA.py`（方案 A）、`align_srt_routeB_llm.py`（方案 B）  
**里程碑标签：** `v13-milestone`
...
**方案 A** 是主力方案：直接把字幕文字送进声学对齐器（wav2vec2），跳过语音识别，从根源上消除专名识别误差。适用于录音与参考文稿高度一致的场景。

**方案 B** 是兜底方案：先用语音识别（faster-whisper）获取"录音里实际说了什么"，再用大语言模型（LLM）做语义层面的匹配。适用于朗读者出现口误、多读、漏读等音频与文稿不一致的场景。
...
两个方案均已支持多语言，通过脚本顶部的 `LANGUAGE` 变量一键切换：

```python
# align_srt_routeA.py / align_srt_routeB_llm.py
LANGUAGE = "es"  # 改为 "fr" 或 "ru" 即可
```

| 语言代码 | 语言 | 方案 A 对齐模型 | 方案 B ASR 模型 |
|---------|------|---------------|---------------|
...
**俄语说明：** WhisperX 未内置俄语对齐模型的自动映射，但脚本已显式指定社区模型 `jonatasgrosman/wav2vec2-large-xlsr-53-russian`，首次运行会自动下载。
...
两套方案共享相同的输入格式，支持 `.m4a`、`.mp3`、`.wav`、`.flac` 音频格式：
...
```python
LANGUAGE = "es"  # 西语。法语改为 "fr"，俄语改为 "ru"
```
...
3. **结果导出**：输出文件位于对应的 `aligned_routeA/` 或 `aligned_routeB/` 目录
...
- **方案 A** 的前提是参考文本与录音高度一致；有错字、漏句时无法自动纠正（此情况应切换方案 B）
- **方案 B** 依赖 LLM API 调用，需要网络连接和 API 费用；且 LLM 裁判并非 100% 可靠，需人工抽检验证
- **超长音频**（单段 > 30 分钟）方案 A 自动分块处理，方案 B 无此限制（faster-whisper 原生支持）
